...
Used stackoverflow to get help
...
#Predicts and displays stock predictions results
def Stock_Predictions(company_ticker):
    ...
    historycal_data = historycal_data.drop(['Dividends', 'Stock Splits'], axis = 1)
    ...
    model.add(LSTM(units=60, activation='relu', return_sequences= True, input_shape = (train_x.shape[1], 1))) #adds everything inside to the Sequential Model
    model.add(Dropout(0.2)) #Drops out in 0.3 seconds

    #Repeat 1 times while omitting and changing slight details
    ...
    test_x = np.array(test_x) #converts the test_x data to an array
    ...
    model.save('kera_model.h4')

#creates the title for the App
st.title("Stock Prediction")

#Asks user for company stock ticker
company_ticker = st.text_input("Please Enter the Company Stock Ticker")

#create an if-statement that will predict and display predictions when giving a ticker
if company_ticker:
    #Predicts the stocks and displas the chart responding to the predictions
    historical_data, prediction = Stock_Predictions(company_ticker)

    #Displays the Stocks History on screen
    st.write("History of Closing Price")
    st.line_chart(historical_data['Close'])

    #Displays the predictions made on screen
    st.write("Predicted Stock Closing Price")
    st.line_chart(pd.DataFrame({"Prediction": prediction}))